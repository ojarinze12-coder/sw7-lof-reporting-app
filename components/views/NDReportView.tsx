...
const NDReportView: React.FC<NDReportViewProps> = ({ ndName, ndId }) => {
  ...
  const data = getAggregatedData(UserRole.NationalDirector, ndId, startDate, endDate);

  useEffect(() => {
    if (isComparing && startDate && endDate) {
        const { prevStart, prevEnd } = getPreviousPeriod(startDate, endDate);
        const prevData = getAggregatedData(UserRole.NationalDirector, ndId, prevStart, prevEnd);
  ...
  const handleExport = () => {
    ...
    link.setAttribute("download", `ND_Report_${ndName.replace(/ /g, '_')}_${startDate}_to_${endDate}.csv`);
  ...
  if (!data) {
    return <Card><p>No data available for {ndName} in the selected period.</p></Card>;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-lg text-slate-600">Welcome, <span className="font-semibold">{ndName}</span></p>
